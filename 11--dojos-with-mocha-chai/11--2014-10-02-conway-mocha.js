  ...
  tick : function (board){
    ...
    board.forEach(function(value, index){
        value.forEach(function(x, y){
           newBoard.push(conway.conway(board[index][y], conway.neighborsOf(board, index, y)));
        });
    });
...
neighbors.forEach(conway.conway);
...
describe('conway.conway', function(){
  it('should return the value of a cell based on its neighbors', function(){
    assert.equal(conway.conway(board[0][0], [board[0][1], board[1][0], board[1][1]]), false);
  });
});
